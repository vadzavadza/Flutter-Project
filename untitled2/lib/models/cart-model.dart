...
class Cart  extends Equatable{
 ...
 double get subtotal =>
     products.fold(0,(total, current)=> total + current.totalAmount);


 double commissionFee(subtotal){
   if(subtotal >= 30.0){
     return 0.0;
   }
   else
 ...
 double totalAmount(subtotal,commissionFee){
   return subtotal + commissionFee(subtotal);
 }

  String feeCommission(subtotal){
   if(subtotal >= 30.0){
     return 'YOU HAVE FREE COMMISSION';
   }
else{
  double missing = 30.0 - subtotal;

  return 'Add \$${missing.toStringAsFixed(2)} for FREE COMMISSION';
   }
  }

  String get subtotalString => subtotal.toStringAsFixed(2);
  String get totalAmountString => totalAmount(subtotal,commissionFee).toStringAsFixed(2);


  String get commissionFeeString => commissionFee(subtotal).toStringAsFixed(2);
  String get feeCommissionString => feeCommission(subtotal);
